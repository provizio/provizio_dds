...
if ! nsx ip link add "$IFACE_BOND" type bond > /dev/null 2>&1; then
    ...
    if [ "$failures" -eq 0 ]; then
        # Phase 1 passed, but the carrier-only case did not run: report ctest SKIP (77,
        # see SKIP_RETURN_CODE) so the coverage gap stays visible instead of counting as
        # a full pass.
        echo "carrier_recovery: phase 1 PASS, phase 2 not covered - SKIP"
        exit 77
    fi
