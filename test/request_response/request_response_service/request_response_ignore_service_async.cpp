...
int main()
{
    constexpr const char *log_prefix = "request_response_ignore_service_async: ";
    ...
    auto service = provizio::dds::make_service<std_msgs::msg::Int32PubSubType, std_msgs::msg::Int64PubSubType>(
        ...
        [&](const std_msgs::msg::Int32 &request) {
            ...
            // Throw immediately for values divisible by 4
            if ((request.data() % 4) == 0)
            {
            ...
            return std::async(std::launch::async, [request]() {
                // Throw inside async for even values not divisible by 4
                if ((request.data() % 2) == 0)
                {
                    throw provizio::dds::ignore_request{};
                }
