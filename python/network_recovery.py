...
def _resolve_env_once() -> bool:
    ...
    with _env_resolution_lock:
        ...
        # Sanitised and quoted, as everywhere else a rejected PROVIZIO_DDS_* value is echoed
        # (and as the C++ counterpart does): an arbitrarily long or control-character-carrying
        # value must not be able to flood the log or forge lines in whatever ingests it.
        _emit_log(
            LogLevel.WARNING,
            f"{_ENV_VAR_NAME}='{_sanitise_env_value_for_log(raw)}' is not recognised "
            f"(use on/off); auto-recovery enabled",
        )
        _env_resolution_cached = True
        return True
...
#
# Lock-guarded, exactly as its C++ counterpart's exchange() is: whichever thread happens to
# be reading the interfaces makes the "first of the streak" decision, and there is more than
# one -- the monitor's worker, plus register_participant, which deliberately builds the
# monitor outside the registry lock on first registration. A bare test-then-set would let
# two of them both conclude they were first and log the same line twice.
_enumeration_failure_reported = False
_enumeration_failure_lock = threading.Lock()
...
def _try_capture_address_snapshot() -> "Optional[AddressSnapshot]":
    ...
    try:
        ...
        with _enumeration_failure_lock:
            first_of_streak = not _enumeration_failure_reported
            _enumeration_failure_reported = True
        # Emitted outside the lock: a user log callback is free to re-enter provizio_dds,
        # and holding a lock across it is how this file's other diagnostics deadlock.
        if first_of_streak:
            _emit_log(
                LogLevel.WARNING,
    ...
    # Read without the lock first: this runs on every polling tick and every kernel-event
    # capture, and in the ordinary case there is no streak to clear. The unlocked read is
    # safe because the write it guards is idempotent -- two threads that both observe True
    # simply both clear it -- and a thread that reads False while another is setting True has
    # observed no failure of its own to clear. Only the transition itself needs the lock, for
    # the reason the failure path takes it.
    if _enumeration_failure_reported:
        with _enumeration_failure_lock:
            _enumeration_failure_reported = False
    return snapshot
...
def _capture_address_snapshot() -> AddressSnapshot:
    ...
    Returns an empty snapshot when the host has no operationally-up non-loopback
    interface -- a legitimate reading, and a different answer from a failed read, which
    raises ``OSError`` instead: see :func:`_try_capture_address_snapshot`, which is what
    every caller that must survive one goes through.
    """
...
def _iter_posix_interface_addresses(
    ...
    if kinds_by_index:
        try:
            ...
            if_nametoindex = None
            # We HAVE kinds and cannot use them: every entry below falls back to "no
            # kind", which is read downstream as "physical hardware". Say so, for the
            # reason spelled out at the per-name lookup below.
            report_interface_kind_lookup_failed()
    ...
    try:
        for entry in _iter_linked_nodes(head, "ifa_next"):
            ...
            if if_nametoindex is not None:
                ...
                # "no kind known" rather than risk a spurious hit -- and SAY so. "No kind"
                # is read downstream as "physical hardware" (_kind_excluded is False for an
                # empty kind), so a failure here silently promotes a tunnel to a real NIC --
                # and this failure is routine rather than exotic: the interface can be
                # renamed or removed between the getifaddrs walk and this lookup, which is
                # exactly what a flapping tunnel does. Mirrors enumerate_interface_addresses
                # in src/address_snapshot_linux.cpp.
                raw_index = int(if_nametoindex(entry.ifa_name))
                if raw_index == 0:
                    report_interface_kind_lookup_failed()
                else:
                    kind = kinds_by_index.get(raw_index, "")
            if entry.ifa_addr:
...
# Guards BOTH the read-and-clear in take_interface_kind_lookup_failure_report and the store in
# report_interface_kind_lookup_failed, whose contract is "at most once per occurrence". The
# take alone is not enough: a report landing between the take's read and its write is
# overwritten by the clear and the occurrence is lost, which the C++ exchange() this mirrors
# cannot do.
_interface_kind_lookup_lock = threading.Lock()
...
def report_interface_kind_lookup_failed() -> None:
    ...
    with _interface_kind_lookup_lock:
...
def take_interface_kind_lookup_failure_report() -> bool:
    ...
    The read and the clear are one critical section, exactly as the C++ counterpart's
    ``exchange`` is: they are separate bytecodes, so without the lock the GIL can switch
    between them and two participants being created concurrently both see the flag set and
    both report -- the repetition the take-once contract exists to prevent. Mirrors
    ``take_interface_kind_lookup_failure_report`` in src/vpn_interfaces.cpp, and matches what
    :func:`take_unmatched_vpn_allow_override_names` two functions away already does."""
    global _interface_kind_lookup_failed
    with _interface_kind_lookup_lock:
...
# IP_ADAPTER_NO_MULTICAST, from iptypes.h: the adapter cannot carry multicast, so Fast-DDS'
# IPFinder skips it entirely (IPFinder.cpp, "Currently not supported interfaces that not
# support multicast"). The allowlist has to agree with IPFinder or the two disagree about how
# many real interfaces the host has.
_IP_ADAPTER_NO_MULTICAST = 0x0010
...
def _iter_windows_adapter_addresses(
    ...
    ``as_ipfinder`` selects the filters Fast-DDS' own ``IPFinder::getIPs`` applies, for the
    one caller that must agree with it exactly -- :func:`allowed_interfaces`, whose answer
    becomes a transport allowlist that Fast-DDS then intersects with what IPFinder found.
    Two differences, both verified against IPFinder.cpp for the pinned Fast-DDS:

    * **No DAD filter.** IPFinder takes every unicast address of an enabled adapter; the
      snapshot path takes only ``IpDadStatePreferred`` ones, matching
      ``capture_address_snapshot``. A deprecated-but-still-bound address was therefore in
      the allowlist Fast-DDS builds and missing from the one this computes.
    * **Skip adapters that cannot multicast.** IPFinder drops ``IP_ADAPTER_NO_MULTICAST``
      (``Flags & 0x0010``); counting such an adapter here inflates the real-interface count,
      which is what decides whether the per-interface netmask filter goes on -- and turning
      it on drops every peer outside the remaining interfaces'' subnets.
    ...
    for adapter_data in _iter_linked_nodes(
        ...
        if adapter_data.OperStatus == _IF_OPER_STATUS_UP and (
            ...
            # IP_ADAPTER_NO_MULTICAST: an adapter Fast-DDS' IPFinder refuses, so the
            # allowlist caller must refuse it too. Left in for the snapshot, which watches
            # what the host HAS rather than what a UDPv4 transport would bind.
            if as_ipfinder and (adapter_data.Flags & _IP_ADAPTER_NO_MULTICAST):
                continue
            for unicast in _iter_linked_nodes(adapter_data.FirstUnicastAddress, "Next"):
                if not as_ipfinder and unicast.DadState != _IP_DAD_STATE_PREFERRED:
...
def _excluded_as_vpn_adapter_win(
    ...
    if not _adapter_is_vpn_win(friendly, description, if_type):
        return False
    # All three are evaluated before they are combined, rather than chained with `and`.
    # _excluded_as_vpn_interface records which PROVIZIO_DDS_ALLOW_VPN_INTERFACES name
    # re-admitted an interface, and take_unmatched_vpn_allow_override_names warns about
    # every name that re-admitted nothing. An `and` chain stops at the first identity the
    # override re-admits, leaving the other two unrecorded -- so a value naming the SAME
    # adapter by two of its identities, its friendly name and its GUID say, would have the
    # second reported as matching no VPN / tunnel interface on this host, sending an
    # operator to hunt a typo in a setting that is working.
    friendly_excluded = _excluded_as_vpn_interface(friendly, platform_says_vpn=True)
    name_excluded = _excluded_as_vpn_interface(name, platform_says_vpn=True)
    description_is_excluded = _excluded_as_vpn_interface(description, platform_says_vpn=True)
    return friendly_excluded and name_excluded and description_is_excluded
...
# Per-thread cache behind :func:`vpn_interface_blocklist_entries` and
# :func:`allowed_interfaces`, active only inside a :func:`scoped_vpn_interface_cache`
# block. One network event rebuilds every recovery-enabled participant in the process, and
# each rebuild otherwise re-runs a full getifaddrs walk plus a full netlink RTM_GETLINK
# dump -- twice, once per enumeration -- to answer the same questions about the same host
# at the same instant; this collapses each of them to one reading.
#
# Both readings, not just the blocklist, because the two are one decision: what is blocked
# and what is left over are applied to the same transports, and the leftovers decide the
# per-interface netmask filter that keeps a blocklist from becoming a sender socket per
# interface. Taking them from two different views of a host that moved mid-pass would leave
# one event's participants configured differently from each other.
#
# Thread-local on purpose, exactly as the C++ scoped_vpn_interface_cache is: a participant
...
_interface_cache = threading.local()


@contextlib.contextmanager
def scoped_vpn_interface_cache() -> "Iterator[None]":
    """Collapse the host enumerations behind :func:`vpn_interface_blocklist_entries` and
    :func:`allowed_interfaces` to at most one each for the duration of the block, on this
    thread only.

    Mirrors ``provizio::dds::detail::scoped_vpn_interface_cache``."""
    depth = getattr(_interface_cache, "depth", 0)
    _interface_cache.depth = depth + 1
    try:
        yield
    finally:
        _interface_cache.depth -= 1
        if _interface_cache.depth == 0:
            ...
            # them.
            _interface_cache.entries = None
            _interface_cache.host_addresses = None


def prime_vpn_interface_cache() -> None:
    """Fill an open :func:`scoped_vpn_interface_cache` with both host readings up front.

    For the caller that opens the scope around a pass of participant rebuilds: a
    participant resolves the exclusion with its own lifecycle lock held, so a reading taken
    there stalls that participant for the length of an rtnetlink round trip, whereas one
    taken here -- participants still live and unlocked -- blocks nobody.

    A no-op outside a cache scope, and silent about a failed reading: nothing is cached for
    one, so whoever asks next reads the host again and handles the failure where it has
    something to do about it. Mirrors ``detail::prime_vpn_interface_cache``."""
    if getattr(_interface_cache, "depth", 0) == 0:
        return
    vpn_interface_blocklist_entries()
    # The blocked set is irrelevant to what this fills: the cache holds the raw reading of
    # the host, which every filtering of it then shares.
    allowed_interfaces(())
...
def vpn_interface_blocklist_entries() -> "FrozenSet[str]":
    ...
    if getattr(_interface_cache, "depth", 0) != 0:
        cached = getattr(_interface_cache, "entries", None)
    ...
    try:
        ...
        if getattr(_interface_cache, "depth", 0) != 0:
            ...
            _interface_cache.entries = entries
        return entries
    except Exception:
...
def _host_interface_addresses() -> "List[Tuple[str, str]]":
    """Every ``(interface name, address text)`` the host reports, loopback included, as a
    UDPv4 transport would enumerate them -- taken from the open
    :func:`scoped_vpn_interface_cache` where there is one, and read from the host
    otherwise.

    Raises whatever the platform walk raises. A failed reading is never cached, for the
    reason a failed blocklist reading is not: a cache exists to collapse one network
    event's repeated enumerations into one reading of the host, while caching a failure
    would hand every participant rebuilt for that event the same wrong answer. Mirrors the
    IPFinder half of ``detail::scoped_vpn_interface_cache``."""
    if getattr(_interface_cache, "depth", 0) != 0:
        cached = getattr(_interface_cache, "host_addresses", None)
        if cached is not None:
            return cached
    ...
    if sys.platform == "win32":
        found = [
            ...
            for name, _friendly, _description, _if_type, addr, _prefix in (
                _iter_windows_adapter_addresses(include_loopback=True, as_ipfinder=True)
            )
        ]
    else:
        found = [
    ...
    if getattr(_interface_cache, "depth", 0) != 0:
        _interface_cache.host_addresses = found
    return found
...
def allowed_interfaces(blocked: "Collection[str]") -> "List[Tuple[str, bool]]":
    ...
    seen: set = set()
    try:
        walk = _host_interface_addresses()
    except Exception:
...
class ListenerDrain:
    ...
    def __init__(self, stall_warning_period: float = _DRAIN_STALL_WARNING_SEC) -> None:
        ...
        Clamped to a positive value: a zero or negative period would make
        Condition.wait return immediately, turning the diagnostic into a busy
        loop that spins a core for as long as the drain lasts and floods the log.
        Mirrors the same clamp in the C++ listener_drain constructor."""
        self._lock = threading.Lock()
        self._cv = threading.Condition(self._lock)
        self._in_flight = 0
        self._detached = False
        self._stall_warning_period = (
            stall_warning_period if stall_warning_period > 0.0 else 0.001
        )
...
class _NetworkRecoveryCoordinator:
    ...
    def _on_safety_net_tick(self) -> None:
        ...
        try:
            # One reading of the host for the whole pass, exactly as the change-driven pass
            # below takes -- a retry rebuilds participants no differently, so two of them
            # retried in the same tick must not be configured from two views of the host.
            # The C++ apply_reset opens its scope before it branches on reset_scope, so it
            # covers retry_only too.
            with scoped_vpn_interface_cache():
                prime_vpn_interface_cache()
                for hook in retry:
                    try:
    ...
    def _on_network_event(self, old: AddressSnapshot, new: AddressSnapshot,
        ...
        # try/finally, because everything between the flag being set and cleared can throw:
        # _live_hooks() allocates, and scoped_vpn_interface_cache() /
        # prime_vpn_interface_cache() read the host. Without it, one MemoryError left
        # _reset_in_progress stuck True and every wait_for_idle() hung for the life of the
        # process -- silently, because the caller one level up swallows the exception and
        # only logs. The C++ coalescer_loop wraps its equivalent run_reset() call for exactly
        # this reason; this is the Python mirror of that guard.
        try:
            ...
            # event's participants configured from two views of the host. Primed up front so no
            # participant pays for a reading while it holds its own lifecycle lock.
            with scoped_vpn_interface_cache():
                prime_vpn_interface_cache()
                for hook in live:
                    try:
    ...
    def inject_transient_for_test(self) -> None:
        ...
        # Through the wrapper, as every other live capture is: a read that fails raises,
        # and this case does not need the real addresses -- the synthetic one alone carries
        # the shape being injected -- so an unreadable interface list degrades to that one
        # entry rather than propagating an OSError out of a test helper.
        captured = _try_capture_address_snapshot()
        end_state = frozenset(captured if captured is not None else ()) | {synthetic}
