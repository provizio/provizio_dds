...
namespace provizio::msg
{
    ...
    class nav_sat_fix_with_headingPubSubType;
}  // namespace provizio::msg
...
namespace provizio::dds
{
     ...
     *
     * @warning This default CHANGED: raw @c Image writers were RELIABLE before this release. Reliability is
     * RxO, and a provizio_dds subscriber's match-publisher default adopts the reliability of the FIRST writer
     * it discovers on the topic and keeps it for that subscriber's life (see @c match_publisher_reliability_qos
     * and the writer-reliability tracking in @c domain_participant). So during a MIXED-VERSION rollout on a
     * shared @c Image topic -- one publisher upgraded, one not -- a subscriber that meets the older RELIABLE
     * writer first builds a RELIABLE reader, and the upgraded BEST_EFFORT writer then never matches it, for
     * that subscriber's life. The same happens within one version to anyone taking the first bullet's advice
     * on a shared topic: an explicit RELIABLE writer discovered first fixes every match-mode reader RELIABLE.
     * It is silent -- an RxO mismatch produces no log line and no matched-count change, so it presents as a
     * subscriber whose callback simply never fires. Upgrade every publisher on one @c Image topic together, or
     * pin the reliability explicitly on both ends while the versions are mixed.
     */
    ...
    // The heading-bearing sibling of NavSatFix, and the same shape and the same producers:
    // a Header plus four float64s, keyless, published standalone by whichever GNSS source
    // can supply heading. A consumer is told to prefer it where it is present, so leaving it
    // at the primary template's reader depth of 1 while NavSatFix reads 32 would put the
    // better of the two topics on the shallower history -- and an overrun there is silent.
    template <>
    struct qos_defaults<::provizio::msg::nav_sat_fix_with_headingPubSubType> final : detail::fleet_shared_qos_defaults
    {
    };
