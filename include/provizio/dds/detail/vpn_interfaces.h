...
namespace provizio::dds::detail
{
     ...
     * host's DDS traffic with it — the worst outcome this feature can produce.
     *
     * A POSIX device name can be talked into it too (`ip link set dev eth0 name wg-mgmt`, or
     * a udev NAME= rule, both routine for giving a NIC a stable custom name in an embedded
     * integration), and the prefix test is a plain lower-cased `starts_with`, not a
     * whole-word match. The difference is one of likelihood, not of possibility: a friendly
     * name is renamed casually and often, a device name deliberately and rarely. Both have
     * the same recourse — PROVIZIO_DDS_ALLOW_VPN_INTERFACES, and the log line naming what
     * was excluded.
     *
    ...
    /**
     * @brief Collapses BOTH host enumerations the VPN exclusion runs — the one behind
     * @c vpn_interface_blocklist_entries and the one behind @c vpn_allowed_interfaces — to
     * (at most) one each for as long as this object lives, on the thread that created it.
     *
     * A single network event rebuilds every recovery-enabled participant in the process,
     * and each rebuild resolves the exclusion again — so the same rtnetlink dump (or
     * @c GetAdaptersAddresses call) and the same @c IPFinder walk ran once per participant
     * to answer one question about one host. With this held for the pass, each runs once.
     *
     * Both, not just the blocklist, because the two are one decision. What is blocked and
     * what is left over are applied to the same descriptors, and the leftovers decide the
     * per-interface netmask filter that keeps a blocklist from turning into a sender socket
     * per interface. Reading them from two different views of a host that moved mid-pass
     * would leave one event's participants configured differently from each other for what
     * is meant to be one atomic response to one network change.
     *
     * A failed reading is never cached, for either: a cache exists to collapse one event's
     * repeated enumerations into one reading of the host, while caching a failure would
     * instead hand every participant rebuilt for that event the same wrong answer.
     *
     ...
     * one's cache.
     */
    class PROVIZIO_DDS_API scoped_vpn_interface_cache final
    {
      public:
        scoped_vpn_interface_cache() noexcept;
        ~scoped_vpn_interface_cache();
        scoped_vpn_interface_cache(const scoped_vpn_interface_cache &) = delete;
        scoped_vpn_interface_cache &operator=(const scoped_vpn_interface_cache &) = delete;
        scoped_vpn_interface_cache(scoped_vpn_interface_cache &&) = delete;
        scoped_vpn_interface_cache &operator=(scoped_vpn_interface_cache &&) = delete;
    };

    /**
     * @brief Fills an open @c scoped_vpn_interface_cache with both host readings up front.
     *
     * For the caller that opens the scope around a pass of participant rebuilds: a
     * participant resolves the exclusion with its own reset lock held exclusively, so a
     * reading taken there stalls every concurrent @c publish(), @c take() and
     * @c register_topic() on it for the length of an rtnetlink round trip. Taken here, with
     * the participants still live and unlocked, it blocks nobody.
     *
     * A no-op outside a cache scope, and silent about a failed reading: nothing is cached
     * for one, so whoever asks next reads the host again and handles the failure where it
     * has something to do about it.
     */
    PROVIZIO_DDS_API void prime_vpn_interface_cache();
