...
namespace provizio::dds::detail
{
    ...
    class network_recovery_coordinator
    {
      ...
      private:
        ...
        /// Guards the two test-override members below. A leaf: taken by nothing else, and
        /// never while another lock of this class is held.
        ///
        /// The overrides are documented idle-only, and that is not enough on its own.
        /// @c wait_for_idle waits for no pending burst and no reset in progress, which it
        /// cannot extend to the kernel: @c on_kernel_event calls @c current_snapshot BEFORE
        /// it takes @c coalescer_mutex, so a test thread writing an override genuinely races
        /// the notification thread reading it -- a veth address event on a container-enabled
        /// runner is enough, and the TSan CI job runs this suite. Both setters are exported
        /// and guarded by no ifdef, so they ship. In production neither override is ever set:
        /// the lock is uncontended and the snapshot copy below never happens.
        std::mutex test_override_mutex;

        /// Set only by @c force_snapshot_for_test, and empty in every non-test build path.
        /// Read by @c current_snapshot from both the coalescer thread and the
        /// kernel-notification thread. Guarded by @c test_override_mutex.
        ...
        /// How many reads one burst may spend on its START snapshot before giving up on it.
        /// The first event of a burst spends attempt 1; a failed read is retried on later
        /// events of the SAME burst, because one failure used to leave the burst with no start
        /// snapshot for its whole life and so disable transient-flap detection exactly when a
        /// flap may be what is happening.
        ///
        /// Bounded, and for a different reason than the end read is: this retry rides on the
        /// kernel event stream rather than on a timer, so a host whose interface list is
        /// durably unreadable while its links churn -- container/veth noise raises a burst per
        /// event -- would pay one full enumeration per event, multiplying syscall load exactly
        /// while the resource that enumeration needs (descriptors, memory) is what is short.
        /// Four attempts mirrors the end read's one initial read plus max_burst_end_read_retries
        /// retries; past that the burst runs without a start snapshot, which is what it did
        /// before the retry existed, and the periodic safety-net tick still runs. Guarded by
        /// coalescer_mutex.
        static constexpr unsigned int max_burst_start_read_attempts{4};

        /// Reads spent on the current burst's start snapshot (see
        /// max_burst_start_read_attempts). Cleared whenever a new burst starts, so the bound is
        /// per-burst rather than per-process. Guarded by coalescer_mutex.
        unsigned int burst_start_read_attempts{0};

        /// How many times a burst whose END snapshot could not be read may be re-armed to
        /// try that read again. Without any re-arm the burst is simply dropped: it has
        /// already been consumed by the time run_reset reads the interfaces, and
        /// last_known_snapshot is deliberately left alone, so nothing schedules another
        /// look -- and for the transient flap this machinery exists to catch, the end state
        ...
        /// max_burst_end_read_retries). Cleared whenever a read succeeds and whenever a new
        /// burst starts, so the bound is per-burst rather than per-process. Guarded by
        /// coalescer_mutex.
        unsigned int burst_end_read_retries{0};
