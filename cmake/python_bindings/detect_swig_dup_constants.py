...
# can be qualified (`std::uint32_t`, `uint8_t`, etc.). The value is captured so
# a same-named constant in an unrelated package can be told apart from a true
# msg/srv mirror (same value) — only the former is a silent loss worth warning.
_CONST_DECL_RE = re.compile(
    r"^\s*const\s+[A-Za-z0-9_:]+\s+(?P<name>[A-Z_][A-Z0-9_]*)\s*=\s*(?P<value>[^;]+);",
    re.MULTILINE,
)
...
def _collect_constants(hpp_path: Path) -> list[tuple[str, str, str]]:
    """Return a list of ``(namespace_name, constant_name, value)`` tuples
    extracted from a generated header."""
    ...
    for ns_match in _NS_BLOCK_RE.finditer(content):
        ...
        for const_match in _CONST_DECL_RE.finditer(ns_match.group("body")):
            constants.append((ns, const_match.group("name"), const_match.group("value").strip()))
    return constants
...
def main() -> int:
    ...
    # const_name -> (first_fq, first_value)
    seen_first: dict[str, tuple[str, str]] = {}
    ...
    for idl_path in idl_files:
        ...
        if pkg_kind is None:
            continue
        pkg, kind = pkg_kind
        for ns, const_name, value in _collect_constants(hpp_path):
            fq = f"{pkg}::{kind}::{ns}::{const_name}"
            prev = seen_first.get(const_name)
            if prev is None:
                seen_first[const_name] = (fq, value)
            elif prev[0] != fq:
                duplicates.add(fq)
                # The dedup key is the bare name, so a legitimately-distinct
                # same-named constant in an unrelated package would be silently
                # %ignore'd. Warn when the suppressed occurrence isn't a
                # same-value mirror — that's a genuine cross-package collision,
                # not the intended ROS msg/srv duplication.
                if value != prev[1]:
                    print(
                        f"warning: constant '{const_name}' = '{value}' in {fq} collides with "
                        f"'{const_name}' = '{prev[1]}' in {prev[0]} (different value); "
                        f"suppressing {fq} from the Python module",
                        file=sys.stderr,
                    )
