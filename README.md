...
- **Recompile every consumer against this release; do not drop the new shared library beside binaries built against an older header.** `domain_participant` has no pimpl -- its full layout, constructor included, lives in the public header -- and this release adds private members to it. That changes `sizeof` and every member offset, and because `participant_generation()` and its neighbours are inline and reached from the header-only templates in `publisher.h` / `subscriber.h`, a consumer's *own compiled* offsets into the class change too, not merely its allocation size. Mixing a new `libprovizio_dds` with objects compiled against an older header is undefined behaviour that shows up as corruption rather than as a link error.
