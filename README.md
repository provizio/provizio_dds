...
| `PROVIZIO_DDS_NETWORK_RECOVERY_SAFETY_NET_SEC` | `30` | Cadence of the periodic re-verification (and notification-channel revival). `0` disables it, leaving recovery purely event-driven. Values above one day are clamped. Read by C++ everywhere and by Python on its event-driven (Linux) backend; the Python polling backends re-verify every `..._POLL_INTERVAL_SEC` instead, since for a poller that *is* the periodic check. |
