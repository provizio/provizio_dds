...
- Optionally, the client request API lets you enforce a post-match settling window and (if desired) a finite matching timeout before the first publish. See C++ `provizio::dds::request(..., std::chrono::milliseconds post_match_delay = 0ms, std::chrono::milliseconds service_match_timeout = 0ms)` for the full set of knobs, and Python `provizio_dds.request(..., post_match_delay_sec=0.0)` for the settling window control.
