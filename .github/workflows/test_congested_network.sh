...
build_images() {
    ...
    while true; do
        ...
        # Run in a subshell with pipefail so the pipeline reports docker's failure rather than
        # tee's success -- without it the status would be tee's, and every failed build would
        # read as a success. "|| status=$?" keeps `set -e` from aborting before the status is
        # read. (PIPESTATUS would work too, but only while nothing runs in between: a trailing
        # "|| true" would itself reset it, silently restoring the very bug this avoids.)
        status=0
        (
            set -o pipefail
            COMPOSE_FILE=congested_network_compose.yml docker compose --progress plain build 2>&1 | tee "${log}"
        ) || status=$?
        if [[ "${status}" -eq 0 ]]; then
            rm -f "${log}"
            return 0
