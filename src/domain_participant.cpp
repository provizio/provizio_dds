...
namespace provizio::dds
{
    namespace
    {
        ...
        // Ceiling on PROVIZIO_DDS_UDP_SOCKET_BUFFER_SIZE. Not a taste judgement about how big a
        // socket buffer should be: Fast-DDS derives the shared-memory segment from it as
        // max(send, listen) x 2 in uint32, so anything above this WRAPS -- 2 GiB lands on a
        // segment_size of 0, and 3 GiB on a real 2 GiB /dev/shm allocation per participant. The
        // Python applies the same ceiling in _resolve_udp_socket_buffer_size, so both
        // languages cap the value they resolve rather than only the one product one of them
        // happens to write into a generated profile.
        constexpr std::uint32_t max_udp_socket_buffer_size = 0x7FFFFFFFU;
        ...
        std::uint32_t resolve_udp_socket_buffer_size()
        {
            ...
            {
                const std::uint32_t parsed = detail::parse_positive_u32(env);
                if (parsed > max_udp_socket_buffer_size)
                {
                    log_warning() << "PROVIZIO_DDS_UDP_SOCKET_BUFFER_SIZE='" << detail::sanitise_env_value_for_log(env)
                                  << "' exceeds the largest usable value " << max_udp_socket_buffer_size
                                  << " (above it, Fast-DDS' shared-memory segment size overflows and the transport "
                                     "ends up with none); using that instead";
                    return max_udp_socket_buffer_size;
                }
                if (parsed != 0U)
                {
                    return parsed;
          ...
          configured_transport_mode(transport), registered_topics_mutex(std::make_shared<std::mutex>())
    {
        ...
        {
            // Non-throwing overload, as the working-directory probe above already uses and for
            // the same reason: the path comes from the environment, so EACCES, ELOOP or
            // ENAMETOOLONG on it are ordinary inputs. The throwing form turned each of them into
            // a std::filesystem_error out of make_domain_participant(), which documents
            // std::runtime_error, and "the profile cannot be read" is anyway just "no profile".
            std::error_code file_error;
            used_xml_profile = std::filesystem::is_regular_file(std::filesystem::path{file_path}, file_error);
        }
        ...
        if (!used_xml_profile)  // Unless configured via the XML profile
        {
            ...
            if (!std::getenv("FASTDDS_BUILTIN_TRANSPORTS"))  // NOLINT: getenv required
            {
                ...
                if (transport == transport_mode::localhost_only)
                {
                    ...
                    // OR-ed in, never assigned. These are BITS
                    // (NO_FILTER/FILTER_DIFFERENT_HOST/FILTER_DIFFERENT_PROCESS/FILTER_SAME_PROCESS),
                    // and cached_qos already carries whatever the caller set through their XML
                    // profile or set_default_participant_qos. Assigning threw that away: a caller
                    // using the documented intraprocess-only recipe
                    // (FILTER_DIFFERENT_HOST | FILTER_DIFFERENT_PROCESS) silently lost the second
                    // half and began accepting other local processes again, having already had
                    // their transports built for intraprocess-only by setup_transports above.
                    auto &ignore_flags = cached_qos.wire_protocol().builtin.discovery_config.ignoreParticipantFlags;
                    ignore_flags = static_cast<eprosima::fastdds::rtps::ParticipantFilteringFlags>(
                        static_cast<std::uint32_t>(ignore_flags) |
    ...
    void domain_participant::trigger_network_recovery_reset()
    {
        ...
        const scope_exit flush_vpn_blocklist_log{[this] { flush_pending_vpn_blocklist_log(); }};
        {
            ...
            // re-registration below allocates -- and the old participant is already deleted, so
            // an escape used to leave this object permanently inert with nothing scheduling
            // another attempt: needs_network_recovery_retry() stayed false, so the safety-net
            // tick never revisited it. Setting the flag first inverts that: a throw leaves it
            // set, which is exactly "retry me".
