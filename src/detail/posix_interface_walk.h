...
namespace provizio::dds::detail
{
    ...
    std::unordered_set<std::string> vpn_blocklist_entries_from(const entries_t &entries, is_vpn_fn is_vpn)
    {
        ...
        for (const auto &entry : entries)
        {
            ...
            {
                // ASCII-only, matching the Python mirror's `name.isascii()` guard and for its
                // reason: a device name reaches Fast-DDS through a generated XML profile on
                // that side, where a byte outside ASCII cannot be represented and the whole
                // document is rejected -- so the exclusion fails open and every tunnel is
                // bound. Nothing here generates XML, but the two sides must agree on WHICH
                // interfaces are excluded, and a non-ASCII-named tunnel excluded by one and
                // not the other is the one disagreement neither may produce. The addresses
                // still carry it either way: they are always ASCII.
                if (!entry.name.empty() && std::none_of(entry.name.begin(), entry.name.end(), [](const char chr) {
    ...
    template <typename on_address_fn> bool walk_posix_interface_addresses(on_address_fn on_address)
    {
        ...
        {
            ifaddrs *head;
            // Declared rather than left to aggregate initialisation: a class with deleted
            // copy/move constructors is still an aggregate in C++17 but not in C++20, where
            // `owner{ifa_head}` below becomes ill-formed. It only bites on instantiation,
            // this being a local class in a function template, so the header alone compiles
            // clean at either standard and CMakeLists.txt pins C++17 -- a trap for a future
            // standard bump rather than a present error, and one line to close.
            explicit ifaddrs_owner(ifaddrs *owned_head) : head{owned_head}
            {
            }
