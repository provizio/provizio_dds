...
namespace provizio::dds::detail
{
    namespace
    {
        ...
        {
            return std::any_of(excluded_name_prefixes.begin(), excluded_name_prefixes.end(),
                               [&name](const std::string_view prefix) { return starts_with(name, prefix); });
        }
        ...
        // Issues a single RTM_GETLINK dump request and parses IFLA_LINKINFO out of every
        // reply. The map returned is keyed by the ifinfomsg's ifi_index -- NOT by name; the
        // caller resolves a name to an index with if_nametoindex, and reports a failure to do
        // so, because a missing entry is indistinguishable from a genuine "no kind" and the
        // two mean opposite things (see enumerate_interface_addresses).
        ...
        std::vector<interface_entry> enumerate_interface_addresses(bool *const enumeration_failed = nullptr)
        {
            ...
            const bool readable =
                walk_posix_interface_addresses([&entries, &kinds_by_index](posix_interface_address address) {
                    ...
                    if (raw_idx == 0)
                    {
                        // ...and SAY so. "No kind" is read downstream as "physical hardware"
                        // (kind_excluded returns false for an empty kind), so a failure here
                        // silently promotes a tunnel to a real NIC -- and this failure is
                        // routine rather than exotic: the interface can be renamed or removed
                        // between the getifaddrs walk and this lookup, which is exactly what a
                        // flapping tunnel does. Without the report, no_link_kinds' promise that
                        // the fallback is "never silent" held only for a failed dump, and the
                        // participant-side warning never fired for this path.
                        report_interface_kind_lookup_failed();
                    }
    ...
    bool snapshot_policy_excludes_interface(const interface_identity &identity)
    {
        ...
        // detail/snapshot_policy.h. Linux's contribution is the rtnetlink IFLA_INFO_KIND:
        // it catches a tunnel whose device someone renamed, and it distinguishes container
        // plumbing (veth, bridge) from a physical NIC, which reports no kind at all.
        const bool platform_says_vpn = is_vpn_interface_kind(identity.kind);

        return snapshot_policy_excludes(
            /*excluded_as_vpn=*/[&] { return excluded_as_vpn_interface(identity.name, platform_says_vpn); },
            /*is_vpn=*/[&] { return platform_says_vpn || is_vpn_interface_name(identity.name); },
