...
namespace provizio::dds::detail
{
...

        {
            ...
            if (!monitor && !coalescer_thread.joinable())
            {
                ...
                catch (const std::exception &exception)
                {
                    ...
                    // Reset the coalescer's request flags so a future re-init starts from a
                    // clean state. OUTSIDE the joinable() guard, which is the only case that
                    // can actually reach here with state to clear: the thread construction is
                    // the LAST statement of the try, so a throw means it was never assigned and
                    // joinable() is false -- while the monitor opened two statements earlier is
                    // already running and its worker can have called on_kernel_event() during
                    // the snapshot read, setting has_pending_burst and a burst_start_snapshot
                    // mid-change. Left set, the NEXT coordinator's first iteration finds a burst
                    // whose timestamps are long past, consumes it at once, and rebuilds every
                    // participant in the process for a change that was already over.
                    {
                        const std::lock_guard<std::mutex> coalescer_lock{coalescer_mutex};
                        ...
                        burst_start_snapshot.clear();
                        burst_end_read_retries = 0;
                        burst_start_read_attempts = 0;
                    }
                    monitor.reset();
    ...
    void network_recovery_coordinator::force_enumeration_failure_for_test(const bool fail)
    {
        ...
        //
        // Under test_override_mutex because idle-only is not enough: on_kernel_event reads
        // this through current_snapshot before taking coalescer_mutex, and no wait_for_idle
        // can stop the kernel delivering an event. See the member's declaration.
        const std::lock_guard<std::mutex> lock{test_override_mutex};
    ...
    {
        // Under test_override_mutex: idle-only by contract, but the contract cannot cover
        // a kernel event arriving mid-write. See the member's declaration.
        const std::lock_guard<std::mutex> lock{test_override_mutex};
    ...
    std::optional<address_snapshot> network_recovery_coordinator::current_snapshot(std::string *deferred_warning)
    {
        ...
        // Both test overrides are read once, under their own leaf mutex, because the
        // threads that write them race the ones that read them however idle the coordinator
        // is said to be (see test_override_mutex). Uncontended and copy-free in production,
        // where neither is ever set.
        bool enumeration_failed = false;
        std::optional<address_snapshot> forced_snapshot;
        {
            const std::lock_guard<std::mutex> lock{test_override_mutex};
            enumeration_failed = forced_enumeration_failure_for_test;
            if (!enumeration_failed)
            {
                // Copied, not referenced: a reference would outlive the lock.
                forced_snapshot = forced_snapshot_for_test;
            }
        }
        if (forced_snapshot)
        {
    ...
    void network_recovery_coordinator::on_kernel_event()
    {
        const auto now = std::chrono::steady_clock::now();
        {
            ...
            if (!has_pending_burst)
            {
                ...
                has_pending_burst = true;
                // A kernel event opens a genuinely new burst, so the end-read bound belongs
                // to it and not to whatever the previous one spent (see run_reset). A burst
                // re-armed there keeps its count, which is what bounds it.
                burst_end_read_retries = 0;
                // Likewise the start-read bound: this burst has spent none of it yet.
                burst_start_read_attempts = 0;
            }
            last_event_time = now;
        }
        ...
        // Attempted on the first event of a burst and, if that read failed, again on any
        // later event of the SAME burst -- the read is retried rather than attempted exactly
        // once. One failure used to leave the burst with no start snapshot for its whole life,
        // which disables transient-flap detection precisely when a flap may be what is
        // happening: for a genuine flap the end state equals the baseline, so
        // added == removed == 0 and every other route through decide_and_apply concludes
        // nothing happened. Costs nothing in the normal case, where the first event's read
        // succeeds and every later event of the burst finds a start already taken.
        //
        // Bounded by max_burst_start_read_attempts, because the retry rides on the kernel
        // event stream: without a bound, a durably unreadable interface list plus a churning
        // host would spend one full enumeration on every event of the burst.
        bool burst_start_wanted = false;
        {
            const std::lock_guard<std::mutex> lock{coalescer_mutex};
            burst_start_wanted =
                has_pending_burst && !burst_start_valid && burst_start_read_attempts < max_burst_start_read_attempts;
            if (burst_start_wanted)
            {
                // Counted here, under the same lock that decided it, so two events racing
                // cannot both read on the last remaining attempt.
                ++burst_start_read_attempts;
            }
        }
        if (burst_start_wanted)
        {
    ...
    void network_recovery_coordinator::coalescer_loop()
    {
        ...
        while (!stop_requested)
        {
            ...
            {
                // The same guard, and for the same reasons, as the safety-net tick above: this
                // path allocates snapshots and sets (capture_address_snapshot, the
                // last_known_snapshot assignment, apply_reset's live_participants) and emits log
                // lines through a user callback that may throw. Letting either escape this
                // thread function would call std::terminate AND leave reset_in_progress stuck
    ...
    void network_recovery_coordinator::adopt_first_readable_snapshot(const address_snapshot &new_snapshot)
    {
        ...
        {
            // Adopted here, where nothing can throw between the assignment and the return: an
            // empty reading IS the baseline from now on. The non-empty path below defers its
            // assignment past apply_reset for the reason given there.
        ...
        // After the pass, never before -- see decide_and_apply. A throw out of apply_reset with
        // the baseline already moved leaves the next tick comparing against a state that never
        // took effect, and the change is dropped rather than retried.
        last_known_snapshot = new_snapshot;
    }
    ...
    void network_recovery_coordinator::run_reset(const address_snapshot &burst_start, bool had_burst_start)
    {
        ...
        if (!captured)
        {
            ...
            // this actually saw.
            //
            // But the burst has already been consumed by the caller, so returning here
            // would drop it with nothing scheduled to look again: no participant is flagged
            // for retry (that flag means "a rebuild failed", which none did), and the
            // safety-net tick compares the live snapshot against a last_known_snapshot that
            // never moved. For the transient flap this machinery exists to catch, the end
            // state EQUALS that baseline, so the tick short-circuits and the flap is missed
            // for good -- leaving sockets bound to an address that was torn down and
            // re-added. So re-arm the burst, carrying its start snapshot over, and let the
        ...
        {
            // The read succeeded, so the bound is spent on the NEXT burst that needs it,
            // not on whatever this one inherited.
            const std::lock_guard<std::mutex> lock{coalescer_mutex};
            burst_end_read_retries = 0;
        }
                                                        ...
                                                        const change_source source)
    {
        ...
        retry_exhaustion_reported = false;
        apply_reset(reset_scope::all);
        // Adopted only AFTER the pass has run. apply_reset can throw before it touches any
        // participant -- bad_alloc from prime_vpn_interface_cache, or from building the live
        // list -- and moving the baseline first meant the change was then lost for good: the
        // caller's handler logs "the safety-net check will try again", but the tick compares
        // against a baseline that has already moved on and finds nothing to do. Left where it
        // was, the very next tick sees the same difference and retries, which is what that
        // message promises.
        last_known_snapshot = new_snapshot;
    }
    ...
    void network_recovery_coordinator::apply_reset(const reset_scope scope)
    {
        ...
        // One reading of the host for the whole pass rather than one per participant
        // rebuilt: every participant here is being rebuilt for the SAME network change, so
        // they should be configured for the same interface set, and asking the OS again for
        // each of them costs an rtnetlink dump plus an IPFinder walk per participant, to
        // answer a question that has not changed in between.
        const detail::scoped_vpn_interface_cache vpn_interface_cache;

        // Filled HERE, before the first participant takes its reset lock, and not left to
        // the first refresh that needs it -- see prime_vpn_interface_cache. What it reads is
        // what the cache above then hands to every participant below.
        //
        // Deliberately its own reading of the host rather than a share of the
        // current_snapshot() the caller took a moment ago, even though both walk the same
        // interfaces. They are different enumerations answering different questions -- the
        // snapshot filters for what change detection must watch, these filter for what the
        // transports may bind, one of them through Fast-DDS' own IPFinder -- so sharing them
        // means caching a raw platform reading across the very machinery whose correctness
        // rests on two readings being distinct (run_reset compares a burst's start against
        // its end). One extra enumeration per reset DECISION is not worth that: retry passes
        // are gated on any_participant_needs_retry, so an idle tick pays nothing, and the
        // decision that does pay goes on to rebuild every participant in the process.
        detail::prime_vpn_interface_cache();
