...
#include <provizio/msg/nav_sat_fix_with_headingPubSubTypes.hpp>
...
namespace provizio::dds
{
    namespace
    {
        ...
        /// Whether @c pub_sub_type is the REAL generated type rather than a forward declaration
        /// that no longer names anything.
        ///
        /// Every other predicate here uses the type only as a template argument to
        /// @c qos_defaults, which never touches it -- so a generated class renamed or moved to
        /// another namespace left the forward declarations in qos_defaults.h standing as
        /// distinct incomplete classes, their specializations still selected, and every
        /// assertion below still passing. The file claimed to make that a build failure and did
        /// not. @c is_base_of_v requires a COMPLETE type, so a phantom declaration is a hard
        /// error here, which is the guarantee the includes above were added for.
        template <typename pub_sub_type> constexpr bool is_the_generated_type()
        {
            return std::is_base_of_v<eprosima::fastdds::dds::TopicDataType, pub_sub_type>;
        }

        template <typename pub_sub_type> constexpr bool publishes(const PublishModeQosPolicyKind expected_publish_mode)
        {
            return is_the_generated_type<pub_sub_type>() &&
                   qos_defaults<pub_sub_type>::datawriter_publish_mode == expected_publish_mode;
        }
    ...
    static_assert(
        publishes<provizio::msg::nav_sat_fix_with_headingPubSubType>(SYNCHRONOUS_PUBLISH_MODE) &&
            writes_with_reliability<provizio::msg::nav_sat_fix_with_headingPubSubType>(RELIABLE_RELIABILITY_QOS) &&
            reads_with_matched_reliability<provizio::msg::nav_sat_fix_with_headingPubSubType>() &&
            keeps_history<provizio::msg::nav_sat_fix_with_headingPubSubType>(expected_fleet_shared_reader_depth,
                                                                             expected_writer_depth),
        "nav_sat_fix_with_heading must resolve to the synchronous fleet-shared QoS defaults, as its NavSatFix "
        "sibling does");
    ...
    static_assert(none_declares_removed_history_depth<
                      ...
                      geometry_msgs::msg::PolygonInstanceStampedPubSubType,
                      geometry_msgs::msg::PolygonStampedPubSubType, geometry_msgs::msg::TransformStampedPubSubType,
                      provizio::msg::radar_infoPubSubType, provizio::msg::camera_intrinsicsPubSubType,
                      provizio::msg::metadataPubSubType, provizio::msg::nav_sat_fix_with_headingPubSubType>(),
