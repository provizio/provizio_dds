...
namespace provizio::dds::detail
{
    namespace
    {
        ...
        // Per-thread enumeration cache, active only inside a scoped_vpn_interface_cache.
        // depth counts nested scopes so an inner one does not release the outer one's
        // cache; each reading is what its enumeration returned for this scope, filled on
        // first use. Two readings rather than one because the exclusion runs two different
        // enumerations -- the platform walk that finds the tunnels, and IPFinder for what
        // the transports are left binding -- and a participant needs both to be configured.
        struct interface_cache_state
        {
            std::size_t depth{0};
            std::optional<std::unordered_set<std::string>> blocklist_entries;
            std::optional<std::vector<eprosima::fastdds::rtps::IPFinder::info_IP>> host_addresses;
        };

        interface_cache_state &interface_cache()
        {
            thread_local interface_cache_state state;
            return state;
        }

        /// The host's addresses as UDPv4Transport's constructor would enumerate them
        /// (@c IPFinder with loopback included), taken from the open cache scope where
        /// there is one and read from the host otherwise. @p own_reading is where a read
        /// that is not cached lands, so it must outlive the returned pointer.
        ///
        /// Returns @c nullptr when the host could not be asked at all -- never cached, for
        /// the reason a failed blocklist reading is not: a cache exists to collapse one
        /// network event's repeated enumerations into one reading, and caching a failure
        /// would hand every participant rebuilt for that event the same wrong answer.
        const std::vector<eprosima::fastdds::rtps::IPFinder::info_IP> *host_addresses(
            std::vector<eprosima::fastdds::rtps::IPFinder::info_IP> &own_reading)
        {
            auto &state = interface_cache();
            if (state.depth != 0 && state.host_addresses)
            {
                return &*state.host_addresses;
            }

            if (!eprosima::fastdds::rtps::IPFinder::getIPs(&own_reading, true))
            {
                return nullptr;
            }
            ...
            state.host_addresses = std::move(own_reading);
            return &*state.host_addresses;
        }

        // One-way, process-wide, and deliberately not part of interface_cache_state: that
    ...
    scoped_vpn_interface_cache::scoped_vpn_interface_cache() noexcept
    {
        ++interface_cache().depth;
    }

    scoped_vpn_interface_cache::~scoped_vpn_interface_cache()
    {
        auto &state = interface_cache();
        if (--state.depth == 0)
        {
            ...
            // them.
            state.blocklist_entries.reset();
            state.host_addresses.reset();
        }
    }
    ...
    bool is_vpn_description(std::string_view description)
    {
        ...
        return std::any_of(vpn_description_prefixes.begin(), vpn_description_prefixes.end(),
                           [&lowered](const std::string_view prefix) { return starts_with(lowered, prefix); });
    }
    ...
    void prime_vpn_interface_cache()
    {
        if (interface_cache().depth == 0)
        {
            return;
        }

        (void)vpn_interface_blocklist_entries();
        // The blocked set is irrelevant to what this fills: the cache holds the raw reading
        // of the host, which every filtering of it then shares.
        (void)vpn_allowed_interfaces({});
    }
                                                          ...
                                                          bool *enumeration_failed)
    {
        ...
        // will hand a sender socket to, not which ones the host finds interesting. Read
        // through the cache, so one reset pass configures every participant it rebuilds
        // from one reading of the host -- see scoped_vpn_interface_cache.
        std::vector<eprosima::fastdds::rtps::IPFinder::info_IP> own_reading;
        const auto *const found = host_addresses(own_reading);
        if (found == nullptr)
        {
    ...
    std::unordered_set<std::string> vpn_interface_blocklist_entries(bool *const enumeration_failed)
    {
        ...
        if (state.depth != 0)
        {
            if (!state.blocklist_entries)
            {
                ...
                state.blocklist_entries = std::move(entries);
            }
            return *state.blocklist_entries;
        }
